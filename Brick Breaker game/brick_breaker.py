...
class base():
    ...
    def movement(self):  
        ...
        if key[pygame.K_LEFT] and self.Rectangle.left > 0:
            ...
            self.direction = -1
        if key[pygame.K_RIGHT] and self.Rectangle.right < Window_width:
            self.Rectangle.x = self.speed
            self.direction = 1

    def draw_base(self):
        pygame.draw.rect(window, (0,0,255), self.Rectangle)
        pygame.draw.rect(window, (255,255,255), self.Rectangle,1)
...
class ball():
            ...
            for item in row:
                if self.rect.colliderect(item[0]):
                    ...
                    if abs(self.rect.left - item[0].right) < collision_threshold and self.x_speed < 0:
                        self.x_speed *= -1
                    if bricks.brick[count_row][count_item][1] >1:
                        bricks.brick[count_row][count_item][1] -= 1
                    else:
                        bricks.brick[count_row][count_item][0] > (0,0,0,0)

                if bricks.brick[count_row][count_item][0] != (0,0,0,0):
                    brick_destroyed = 0   
                count_item +=1
            count_row +=1
    ...
    def draw(self):
        ...
        pygame.draw.circle(window, (255,255,255), (self.rect.x + self.radius, self.rect.y + self.radius), self.radius,2)
...
while game:
    ...
    pygame.display.flip()   # added for any updates that we make to the gaming window to become visible.
