...
def sql_fetch(con):
    ...
    try:
        # SQL search query
        cursorObj.execute('SELECT DISTINCT * FROM users ORDER BY rowid DESC')
    except Error:
    ...
    first_row = "{:^30}".format("Name") + "{:^40}".format("Profile Link") + "{:^30}".format("Date Range") + \
        "{:^30}".format("Top Repo") + "{:^20}".format("Repo Lang") + \
        "{:^30}".format("Repo Link") + '\n'
    ...
    for row in rows:
        ...
        display_text += (name + profile_link + date_range +
                         repo + repo_lang + repo_link + '\n')
...
# Button creation
scrape_btn = ttk.Button(window, text="Scrape Users!",
                        style='my.TButton', command=scrape_users)
