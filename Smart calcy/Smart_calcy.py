    ...
    for word in text.split(' '):
        if word.upper() in operations.keys():
            try:
                ...
                Answer_box.insert(END,"Somthing went worng, Could you pls retype :')") # message if there is an input error
            finally:
                break
...
win.configure(bg='Sky blue')                # Defining Background color of the calcy

title_lable = Label(win, text="I am your Smart Calcy :D", width=30 , padx=3)   #Defining the titile lable
title_lable.place(x=160,y=10)
intro_lable = Label(win, text="You can call me 'Genius'", padx=3)       #Defining the intro lable
intro_lable.place(x=200,y=40)
ask_lable = Label(win, text="How may I help you? You can type in below I will read it :)", padx=3) #Defining the asking lable
ask_lable.place(x=110,y=130)

text_in = StringVar()
Question_entry = Entry(win, width=30, textvariable = text_in) #Entry to take questions from users
Question_entry.place(x=180,y=160)

Answer_button = Button(win, text="What do you say?" , command = Calculate) #Defining button to ask answer
Answer_button.place(x=220,y=190)

Answer_lable = Label(win, text="Genius says ...", padx=3)
Answer_lable.place(x=150,y=240)

Answer_box = Listbox(win, width= 40, height=3) #Defining area to answer the given question
